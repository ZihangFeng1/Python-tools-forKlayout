...
def create_recursion(layout, cell_id, layer_id, rec_level):
    ...
    if rec_level > 20:
        ...
        edge = pya.Edge(pya.Point(0, 0), pya.Point(3000, 0))
        layout.cell(cell_id).shapes(layer_id).insert_edge(edge)

    else:

        # create a new cell
        new_cell = layout.add_cell("level" + str(rec_level))
        cell = layout.cell(cell_id)

        # and add four instances of it
        insts = [
            pya.CellInstArray(new_cell, pya.DCplxTrans(1.0 / 3.0, 0.0, False, pya.DPoint(0.0, 0.0))),
            pya.CellInstArray(new_cell, pya.DCplxTrans(1.0 / 3.0, 60.0, False, pya.DPoint(1000.0, 0.0))),
            pya.CellInstArray(new_cell, pya.DCplxTrans(1.0 / 3.0, -60.0, False, pya.DPoint(1500.0, 1000 * math.sin(math.pi / 3.0)))),
            pya.CellInstArray(new_cell, pya.DCplxTrans(1.0 / 3.0, 0.0, False, pya.DPoint(2000.0, 0.0)))
        ]
        for i in insts:
            cell.insert(i)
